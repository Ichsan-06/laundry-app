...
@section('title', 'Edit Machine - Laundry Track')
...
<div class="mx-auto max-w-4xl pb-10">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-900">Edit Machine</h2>
            <p class="text-sm font-medium text-slate-400">Update unit information and pricing</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-[32px] bg-white p-8 shadow-sm ring-1 ring-slate-100 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="machine_code" required value="{{ old('machine_code', $machine->machine_code) }}" class="block w-full rounded-2xl border-slate-100 bg-slate-50 py-3.5 px-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex gap-4">
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="machine_type" value="WASHER" class="peer hidden" {{ $machine->machine_type == 'WASHER' ? 'checked' : '' }}>
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="machine_type" value="DRYER" class="peer hidden" {{ $machine->machine_type == 'DRYER' ? 'checked' : '' }}>
                            <div class="flex items-center justify-center gap-2 rounded-2xl border-2 border-slate-50 bg-slate-50 py-4 text-sm font-extrabold text-slate-400 transition peer-checked:border-primary-600 peer-checked:bg-primary-50 peer-checked:text-primary-600">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="brand" value="{{ old('brand', $machine->brand) }}" class="block w-full rounded-2xl border-slate-100 bg-slate-50 py-3.5 px-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-extrabold uppercase tracking-widest text-slate-400">Capacity (KG)</label>
                        <input type="number" name="capacity_kg" required value="{{ old('capacity_kg', $machine->capacity_kg) }}" class="block w-full rounded-2xl border-slate-100 bg-slate-50 py-3.5 px-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="status" class="block w-full rounded-2xl border-slate-100 bg-slate-50 py-3.5 px-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20">
                        ...
                        <option value="IN_USE" {{ $machine->status == 'IN_USE' ? 'selected' : '' }}>In Use</option>
                ...
                <div class="rounded-2xl border border-slate-50 bg-slate-50/50 p-4 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="durations[{{ $key }}][duration_minutes]" value="{{ old('durations.'.$key.'.duration_minutes', $duration?->duration_minutes ?? 30) }}" class="block w-full rounded-xl border-slate-100 bg-white py-2.5 px-3 text-xs font-bold text-slate-900">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-bold text-slate-400 uppercase">Price (Rp)</label>
                            <input type="number" name="durations[{{ $key }}][price]" value="{{ old('durations.'.$key.'.price', $duration?->price ?? 25000) }}" class="block w-full rounded-xl border-slate-100 bg-white py-2.5 px-3 text-xs font-bold text-slate-900">
                        </div>
                    </div>
                </div>
                @endforeach
