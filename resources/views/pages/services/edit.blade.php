...
@section('title', 'Edit Paket Layanan - Laundry Track')
...
<div class="mx-auto max-w-2xl pb-10">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-900">Edit Paket Layanan</h2>
            <p class="text-sm font-medium text-slate-400">Perbarui harga dan detail paket layanan</p>
        </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="satuan" required class="block w-full rounded-2xl border-slate-100 bg-slate-50 py-3.5 px-4 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20">
                        @foreach($opsiSatuan as $value => $label)
                            <option value="{{ $value }}" @selected(old('satuan', $service->satuan ?? \App\Models\ServicePackage::SATUAN_PER_KG) === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
