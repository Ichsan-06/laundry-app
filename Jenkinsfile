        ...
        stage('Deploy ke VPS') {
            steps {
                sshagent(['vps-ssh-credentials']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ubuntu@43.173.1.182 "cd ~/laundry-app-production && docker compose -f docker-compose.dev.yml pull app && docker compose -f docker-compose.dev.yml up -d app"
                    """
                }
            }
        }
